...
function addStock() {
  ...
  if ($con) {
    ...
    if ($row) {
      ...
      $query = "INSERT INTO medicines_stock (NAME, BATCH_ID, EXPIRY_DATE, QUANTITY, MRP, RATE, INVOICE_NUMBER)
                VALUES('$name', '$batch_id', '$expiry_date', $quantity, $mrp, $rate, $invoice_number)";
      mysqli_query($con, $query);
    }
  }
}
...
function addNewPurchase() {
  ...
  // ✅ Validation: Ensure grand_total is numeric
  if (!is_numeric($grand_total)) {
    echo "Error: Invalid grand total value.";
    return;
  }
  ...
  if ($con) {
    $query = "INSERT INTO purchases (SUPPLIER_NAME, INVOICE_NUMBER, PURCHASE_DATE, TOTAL_AMOUNT, PAYMENT_STATUS)
              VALUES('$suppliers_name', $invoice_number, '$invoice_date', $grand_total, '$payment_status')";
...
?>
  ...
  <div class="row col col-md-8">
    <div class="col col-md-4">
      <label for="generic_name" class="font-weight-bold">&nbsp;If new medicine, generic name:</label>
    </div>
